define(['jquery', 'TYPO3/CMS/Backend/Modal'], function($, Modal) {
    ...
    PartnerHandler.init = function() {
        this.container = document.querySelector('.t3js-partners-container');
        ...
        this.partners = document.querySelectorAll('.t3js-partners-container .t3js-item-container');

        this.addRemoveButtons();
        this.addNewButton();
    };

    PartnerHandler.itemCount = function() {
        return this.partners.length;
    }

    PartnerHandler.addRemoveButtons = function() {

        this.container.addEventListener('click', function(ev) {
            let node = ev.target;
            if (! node.classList.contains('t3js-partners-item-remove')) {
                return;
            }
        ...
        this.partners.forEach((item, index) => {
            ...
            item.classList.add('t3js-partner-item-' + index);
            let btn = document.createElement('a');
            btn.classList.add(
                'btn',
                'btn-default',
                'btn-sm',
                't3js-partners-item-remove',
                't3js-iterable'
            );
            btn.style.float = 'right';
            btn.style.marginTop = '-6px';
            btn.dataset.template = 't3js-partner-item-[i]';
            btn.dataset.target = 't3js-partner-item-' + index;
    ...
    PartnerHandler.setIterations = function (item, iteration) {
        ...
        iterables.forEach((item) => {
            switch (item.dataset.iterationTarget) {
                ...
                case 'partner':
                    ...
                    item.dataset.partnerId = iteration;
                    break;
                case 'href':
    ...
    PartnerHandler.addNewButton = function () {

        this.container.addEventListener('click', function(ev) {

            let node = ev.target;
            if (! node.classList.contains('t3js-partners-item-new')) {
                return;
            }

            let itemContainer = this.partners.item(this.partners.length - 1).cloneNode(true);
            ...
            insertAfter(itemContainer, this.partners.item(this.partners.length - 1));
            this.partners = document.querySelectorAll('.t3js-partners-container .t3js-item-container');
        ...
        btn.classList.add('t3js-partners-item-new');
        btn.href = 'javascript:;';
        btn.textContent = 'Partner hinzufügen';
