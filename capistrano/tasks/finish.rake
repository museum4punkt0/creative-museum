namespace :deploy do
    after :published, :frontend do
        ...
        on roles(:all) do
            execute "bash -c \"cd #{release_path}/backend && bin/console doctrine:migrations:migrate\""
            execute "bash -c \"cd #{release_path}/backend && bin/console cache:clear\""
            execute "bash -c \"/var/www/.npm-global/bin/pm2 restart blm_cm\""
            execute "bash -c \"curl https://testbackend.creativemuseum.landesmuseum.de/flush.php\""
        end
    end
end