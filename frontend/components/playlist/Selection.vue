  ...
  <div w:flex="~ col 1">
    <div v-show="step === 1" w:flex="~ col 1">
      ...
      <div w:p="6" w:grid="~ cols-2 gap-6" w:max-h="lg" w:overflow="y-auto" w:overscroll="y-auto">
        <button
          v-for="(item, key) in user.playlists" :key="key"
          class="btn-primary"
        ...
        <button class="btn-primary" @click.prevent="step = 2"  v-if="addButton">
          <span
            ...
            w:justify="center"
          >
            <span class="add-btn-inline" w:display="block" w:text="center" w:m="y-4"></span>
            {{ $t('playlist.new')}}
          </span>
        </button>
      </div>
    ...
    <div v-show="step === 2" w:flex="~ col 1" w:justify="between">
      <div>
        <div w:p="6" class="page-header">
          <a class="back-btn" @click="backLink">{{ $t('playlist.createNew') }}</a>
        </div>
        <div w:p="6">
          <input v-model="newPlaylistName" class="input-text" :placeholder="$t('post.actions.playlist.createNew.placeholder')" />
        </div>
      </div>
      <div w:p="6" w:mt="auto">
        <button class="btn-primary" w:w="full" @click="createPlaylistWithPost">{{ $t('post.actions.playlist.createNew.submitButton') }}</button>
      </div>
    </div>
  </div>
</template>
